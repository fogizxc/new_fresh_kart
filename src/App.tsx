...
import { Heart, LogOut, MapPin, Plus, ShoppingBag, Store, ShieldCheck, Sparkles, AlertCircle } from 'lucide-react';
...
import { BlinkitSuggestions } from './components/BlinkitSuggestions';
      ...
      <header className="sticky top-0 z-40 border-b border-black/5 bg-white/95 backdrop-blur-md shadow-xs">
        ...
        <div className="border-b border-slate-100 py-2.5 px-4">
          <div className="mx-auto flex max-w-[1500px] items-center justify-between">
            {/* Left side: Leaf Logo & 15-Min Delivery badge */}
            <div className="flex items-center gap-2.5">
              <div
                ...
                className="flex h-9 w-9 items-center justify-center rounded-2xl bg-[#173d2e] text-lg shadow-sm cursor-pointer select-none hover:bg-[#123024] transition-colors"
              >
                🌿
              </div>
              <div className="text-xs font-black text-[#173d2e]">
            ...
            <div
              ...
              className="flex items-center cursor-pointer select-none text-center"
            >
              <div>
                <span className="text-xl sm:text-2xl font-black tracking-tight text-[#173d2e] uppercase font-sans">
                  FreshCart
                </span>
      ...
      {role === 'employee' ? (
        ...
        <div className="pt-4">
          <BlinkitSuggestions
            ...
            onBrowseCategory={cat => {
              setCategory(cat as BlinkitCategory);
              setActiveTab('home');
            }}
          />
        </div>
        ...
        */
        <main className="space-y-4">
          ...
          {/* SECTION 2: CHOOSE YOUR LOCAL MERCHANT (4 LOCAL MERCHANTS CLOSER TO USER) */}
          <BlinkitMerchantSelector
            shops={shops}
            selectedShopId={selectedShopId}
            onSelectShop={id => {
          ...
          <section id="fresh-picks" className="mx-auto max-w-[1500px] px-4 pt-8 sm:px-6 lg:px-8">
            <div className="flex items-end justify-between border-b border-slate-200/70 pb-3 mb-6">
              <div>
              ...
              <span className="rounded-full bg-white px-3.5 py-1.5 text-xs font-black text-[#4f6257] border border-slate-200/60 shadow-xs">
                {loading ? 'Refreshing...' : `${filtered.length} products`}
              </span>
            </div>
          ...
          {/* SECTION 5: SUGGESTION FEATURES (AI Smart Bundles & Combos) */}
          <BlinkitSuggestions
            products={products}
            onAddToCart={handleAddToCart}
            onBrowseCategory={cat => {
          ...
          {/* In-app Feature Center (Coupons, Wishlist, Notifications) */}
          <FeatureCenter
            role={role}
            products={products}
            orders={orders}
...
function ProductCard({
  ...
  return (
    <article className="group relative flex flex-col justify-between overflow-hidden rounded-3xl bg-white border border-slate-200/70 p-3 shadow-xs transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
      <div>
        <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-[#f4f7f2] flex items-center justify-center text-4xl">
          {p.imageUrl ? (
            <img
              src={p.imageUrl}
              alt={p.name}
          ...
          <button
            onClick={onLike}
            className="absolute right-2 top-2 rounded-xl bg-white/90 p-1.5 text-[#517362] shadow-xs hover:bg-white transition-colors"
          >
            <Heart size={14} fill={liked ? 'currentColor' : 'none'} />
          </button>
          <span className="absolute bottom-1.5 left-1.5 rounded-md bg-white/95 px-1.5 py-0.5 text-[9px] font-black text-slate-700 shadow-xs">
            {p.unit}
          </span>
        </div>
        ...
        <div className="mt-2.5">
          ...
          <h4 className="mt-0.5 line-clamp-2 min-h-8 text-xs font-black leading-tight text-[#173d2e]">
            {p.name}
          </h4>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2">
        <div>
          <span className="text-sm font-black text-[#173d2e]">₹{p.sellingPrice}</span>
          {p.mrp > p.sellingPrice && (
            <span className="ml-1.5 text-[10px] text-slate-400 line-through">₹{p.mrp}</span>
          )}
        </div>
        <button
          disabled={!p.stock}
          onClick={add}
          className="flex h-8 items-center gap-1 rounded-xl bg-emerald-50 px-2.5 text-xs font-black text-[#173d2e] border border-emerald-200/80 hover:bg-[#173d2e] hover:text-white transition-all active:scale-95 disabled:opacity-40"
        >
          <Plus size={14} />
          <span>ADD</span>
        </button>
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 sm:items-center backdrop-blur-xs">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-[#f7f7f2] p-5 shadow-2xl sm:p-7 border border-slate-100 animate-in fade-in zoom-in-95">
          ...
          <div>
            ...
            {currentShop && (
              <p className="mt-0.5 text-xs text-[#52655b]">
                Fulfilling from <strong>{currentShop.name}</strong> • ETA:{' '}
        ...
        <div className="mt-5 space-y-4">
          ...
          <div>
            <div className="text-xs font-extrabold text-[#315245]">Payment Method</div>
          ...
          <div className="flex items-center justify-between rounded-2xl bg-white p-4 border border-slate-200/60 shadow-xs">
            <div>
              ...
              <p className="text-[10px] text-emerald-700 font-bold">Includes GST & Local Rider charges</p>
            </div>
          ...
          >
            {saving ? 'Confirming Order & Dispatching Rider…' : 'Place Hyperlocal Order Now'}
          </button>
        </div>
      </div>
    </div>
