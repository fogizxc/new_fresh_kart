...
// Production must always use the same-origin Vercel function. A stale or
// incorrectly configured VITE_API_URL causes shopkeeper actions to call a
// non-existent host/path and return 404. Keep the env override for local
// development and explicitly configured absolute API hosts.
...
export async function updateShopkeeperProduct<T = any>(id: string, updates: Record<string, unknown>): Promise<T> {
  return shopkeeperRequest<T>(`/shopkeeper-portal/products/${encodeURIComponent(id)}`, {
    method: 'PATCH', body: JSON.stringify(updates)
  });
}

export async function batchUpdateShopkeeperProducts<T = { success: boolean; updatedCount: number }>(items: Array<Record<string, unknown> & { id: string }>): Promise<T> {
  return shopkeeperRequest<T>('/shopkeeper-portal/products/batch', {
    method: 'PATCH', body: JSON.stringify({ items })
  });
}

export async function createShopkeeperProduct<T = any>(product: Record<string, unknown> & { name: string; category: string; unit: string; mrp: number; sellingPrice: number; stock: number }): Promise<T> {
...
export function getPayrollExportUrl(month: string, type: 'payroll' | 'neft' = 'payroll'): string { const token = localStorage.getItem('freshcart_token') || ''; return `/api/shopkeeper-portal/staff/payroll-export?month=${encodeURIComponent(month)}&type=${encodeURIComponent(type)}&token=${encodeURIComponent(token)}`; }
