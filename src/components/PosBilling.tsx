...
export function PosBilling({ products, orders, shopName = 'FreshCart Partner Store', shopAddress = '', flash }: Props) {
  ...
  const loadOrder = (id: string) => {
    ...
    setCustomer({ name: `Customer ${order.customerId.slice(0, 8)}`, phone: '', address: '' });
    setTax(0); setDiscount(order.discount || 0); setInvoiceNumber('');
  };
    ...
    try {
      ...
      const response = await fetch('/api/invoices', {
        ...
        body: JSON.stringify({
          ...
          customer: { name: customer.name || 'Walk-in Customer', phone: customer.phone, address: customer.address },
          shopName,
          shopAddress,
          items: cartItems.map(({ product, quantity }) => ({ productId: product.id, name: product.name, unit: product.unit, quantity, unitPrice: product.sellingPrice })),
          subtotal,
          tax,
          discount,
          total,
      ...
      flash?.(error instanceof Error ? error.message : 'Invoice could not be saved');
  ...
  return <section className="mt-4 grid gap-5 xl:grid-cols-[1fr_420px] print:block">
    ...
    <div className="rounded-3xl bg-white p-5 shadow-sm print:shadow-none"><div className="flex items-center justify-between gap-2 print:hidden"><h3 className="text-lg font-black text-[#173d2e]">Bill preview</h3><div className="flex gap-2"><button disabled={saving} onClick={() => void saveInvoice()} className="inline-flex items-center gap-2 rounded-xl bg-[#d7ef8d] px-3 py-2 text-xs font-black text-[#173d2e] disabled:opacity-50">{saving ? <Loader2 size={15} className="animate-spin"/> : <Save size={15}/>}Save bill</button><button onClick={printBill} className="inline-flex items-center gap-2 rounded-xl bg-[#173d2e] px-3 py-2 text-xs font-black text-white"><Printer size={15}/>Print bill</button></div></div>
