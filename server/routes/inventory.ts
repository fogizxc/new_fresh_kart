...
inventory.patch('/products/:id/stock', requireAuth, requirePermission(PERMISSIONS.UPDATE_STOCK), async (req, res) => {
  ...
  if (mongoDb()) {
    ...
    if (!isAdmin && (!product.shopId || product.shopId !== req.user?.shopId)) {
      return res.status(403).json({ error: 'Product is outside the assigned shop' });
    }
