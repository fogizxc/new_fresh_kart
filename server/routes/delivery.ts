...
delivery.patch('/orders/:id/status', async (req,res)=>{
  ...
  const providedOtp = req.body?.otp ? String(req.body.otp).trim() : '';

  if(mongoDb()){
    const db=mongoDb()!,order=await db.collection<import('../models/domain').Order>('orders').findOne({id:req.params.id});
    if(!order) return res.status(404).json({error:'Order not found'});
    const isAdmin=['admin','super_admin'].includes(req.user!.role);
    if(!isAdmin&&order.shopId!==req.user!.shopId) return res.status(403).json({error:'Insufficient permissions'});
    const assignment=await db.collection<DeliveryAssignment>('deliveryAssignments').findOne({orderId:order.id});
    if(!assignment) return res.status(409).json({error:'Order has no delivery assignment'});
    if(!isAdmin&&assignment.employeeId!==req.user!.id) return res.status(403).json({error:'Delivery is assigned to another employee'});

    // Proof of Delivery OTP check
    if(status === 'DELIVERED' && order.deliveryOtp && providedOtp !== order.deliveryOtp) {
      return res.status(400).json({ error: 'Invalid delivery OTP. Please verify with customer at doorstep.' });
    }
    ...
    const updated=await updateOrderStatus(order.id,orderStatus);
    const updatedAssignment={...assignment,status,...(status==='DELIVERED'?{deliveredAt:new Date().toISOString()}: {})};
    await upsertDeliveryAssignment(updatedAssignment);
    if(status==='DELIVERED') {
      await createNotification({id:`n-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,userId:assignment.employeeId!,title:'Delivery completed',message:`${order.id} completed — ₹${(assignment.earning??0).toFixed(2)} earned for ${assignment.distanceKm??0} km.`,type:'PAYMENT',read:false,createdAt:new Date().toISOString()});
    }
    void notifyOrderStatusChange(order as any, orderStatus);
    return res.json({...updated,deliveryStatus:status,deliveryEmployeeId:assignment.employeeId,distanceKm:assignment.distanceKm,ratePerKm:assignment.ratePerKm,earning:assignment.earning});
  }
  ...
  if(status==='PICKED_UP'||status==='OUT_FOR_DELIVERY') order.status='OUT_FOR_DELIVERY';
  void notifyOrderStatusChange(order as any, order.status);
