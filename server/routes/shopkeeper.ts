...
import { requireAuth, requirePermission, requireRole } from '../auth/middleware';
...
shopkeeper.get('/orders', requirePermission(PERMISSIONS.VIEW_OWN_SHOP_ORDERS), async (req, res) => {
  const shopId = req.user!.shopId;
  if (mongoDb()) return res.json(await findOrders({ shopId }));
  return res.json(orders.filter(o => o.shopId === shopId));
});
shopkeeper.get('/stock-alerts', requirePermission(PERMISSIONS.VIEW_INVENTORY), async (req, res) => {
  const shopId = req.user!.shopId;
...
shopkeeper.patch('/orders/:id/status', requirePermission(PERMISSIONS.PROCESS_ORDERS), async (req, res) => {
  ...
  if (status === 'CANCELLED' && !req.user?.role) return res.status(403).json({ error: 'Insufficient permissions' });
  if (mongoDb()) {
    ...
    if (status === 'CANCELLED') {
      ...
      if (!['shopkeeper', 'store_manager', 'admin', 'super_admin'].includes(req.user!.role)) return res.status(403).json({ error: 'Only authorized managers can cancel orders' });
...
shopkeeper.patch('/products/:id/stock', requirePermission(PERMISSIONS.UPDATE_STOCK), async (req, res) => {
