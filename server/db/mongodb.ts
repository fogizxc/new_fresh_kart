...
const DEFAULT_PORTAL_ACCOUNTS: Array<{ id: string; name: string; email: string; role: Role; passwordHash: string }> = [
...
async function ensureDefaultPortalAccounts(database: Db) {
  ...
  for (const account of DEFAULT_PORTAL_ACCOUNTS) {
    const existing = await users.findOne({ $or: [{ id: account.id }, { email: account.email }] });
    ...
    await users.insertOne({
      ...
      role: account.role, active: true, passwordHash: account.passwordHash,
    });
...
async function ensureAdminAccount(database: Db) {
  ...
  const user: User = {
    ...
    username: looksLikeEmail ? undefined : identifier, role: 'super_admin', active: true, passwordHash: hashPassword(password),
  };
...
export async function connectMongo() {
  ...
  connectPromise = (async () => {
    ...
    try {
      ...
      try { await ensureDefaultPortalAccounts(nextDb); } catch (error) { console.error('FreshCart default portal account seeding skipped:', error); }
      try { await ensureAdminAccount(nextDb); } catch (error) { console.error('FreshCart super-admin bootstrap skipped:', error); }
      try {
        const shopCount = await nextDb.collection('shops').countDocuments();
        if (shopCount === 0 && process.env.NODE_ENV !== 'production') {
          ...
          console.log('FreshCart development database catalog & slots seeded successfully.');
        }
      } catch (error) { console.error('FreshCart initial data seed skipped:', error); }
      client = nextClient;
      db = nextDb;
      return nextDb;
    } catch (error) {
...
export async function closeMongo() { if (client) await client.close(); client = null; db = null; connectPromise = null; }
